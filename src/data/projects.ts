...
export const projects: Project[] = [
  {
    ...
    description: 'Shared backend powering 6 frontend apps across 2 university tech fests, serving 6,000+ users',
    ...
    problem: 'Two major university tech fests with thousands of participants needed separate but overlapping systems — registration, payments, hospitality, bed allocation, attendance, and leaderboards. Building them separately would mean duplicated effort and inconsistent data.',
    solution: 'Built a shared Go backend serving 6 frontend apps with 27+ REST endpoints across 12 domain modules. Implemented a dual-path bed allocation algorithm with row-level locking, a concurrent email dispatcher with disk-persistent queue, and PayU payment integration with SHA-512 verification. Added Prometheus metrics, k6 load testing, and RabbitMQ for async event publishing.',
    lessons: [
    ...
    stack: ['Go', 'Gin', 'PostgreSQL', 'Redis', 'RabbitMQ', 'PASETO', 'Prometheus', 'React', 'Next.js', 'TypeScript', 'TanStack', 'Tailwind'],
    accent: '#f59e0b',
    size: 'large',
  ...
  },
  {
    id: 'paper-scout',
    title: 'Paper Scout',
    description: 'AI-powered research paper analysis pipeline with vector search and LLM-driven gap detection',
    ...
    problem: 'Finding relevant research papers and identifying gaps in existing literature is tedious. Existing tools list papers but do not analyze relationships, contradictions, or unexplored directions between them.',
    solution: 'Built a 7-stage pipeline with hybrid sync/async execution. Papers are discovered from dual sources, ranked using 768-dim Gemini embeddings combined with LLM re-scoring (70% LLM + 30% embedding weight), and analyzed by 7 specialized LLM agents for gap detection, feasibility scoring, and report generation. Added token bucket rate limiting, exponential backoff, and graceful degradation across all external APIs.',
    lessons: [
    ...
    stack: ['Go', 'Gin', 'PostgreSQL', 'Redis', 'Qdrant', 'Google Gemini', 'GROBID', 'Docker', 'gRPC', 'SSE'],
    accent: '#10b981',
    size: 'large',
  ...
  },
  {
    id: 'vox',
    title: 'Vox',
    description: 'Real-time voice-to-text pipeline with Whisper transcription and async LLM cleanup',
    ...
    problem: 'Existing voice typing tools either have high latency (waiting for LLM cleanup) or produce raw, unpolished text. There is no tool that gives you instant feedback while still delivering cleaned-up output.',
    solution: 'Built a multi-threaded state machine with bounded-queue architecture achieving sub-second transcription. The key insight: paste raw text instantly, then run LLM cleanup asynchronously in a daemon thread. Added Silero VAD for silence trimming, int8 quantization for 6GB GPU compatibility, and an OutputBackend abstraction supporting xdotool paste with context-aware replace strategies.',
    lessons: [
  ...
  },
  {
    ...
    problem: 'Lost-and-found systems rely on text descriptions, which are unreliable for visual items. A lost blue backpack described differently by two people will never match in a text-only search.',
    solution: 'Built an on-device CLIP embedding pipeline that generates 512-dim vectors for every uploaded image, enabling cosine similarity search directly in PostgreSQL via pgvector. Combined with full-text search for hybrid multimodal matching. Implemented PASETO v4 asymmetric auth, S3-compatible image storage with presigned URLs, and a comprehensive 3-layer test suite with 317 test functions.',
    lessons: [
  ...
  },
  {
    ...
    problem: 'Most KV stores are either too complex (RocksDB) or too simple (a map). Building a production-grade storage engine teaches you about disk I/O, concurrency, compaction, and crash recovery — things no framework abstracts away.',
    solution: 'Implemented a Bitcask design with in-memory KeyDir, append-only log segments, auto-rotation at 16MB, and background merge compaction triggered by fragmentation ratio. Added hint files for fast recovery, an LRU file cache to prevent file descriptor exhaustion, and CRC32 integrity checks on every entry. Tested with 53+ test functions including 200-goroutine stress tests.',
    lessons: [
  ...
  },
  {
    ...
    problem: 'Tracking watched movies and TV shows across platforms is fragmented. Existing apps either lack customization or require manual entry. Wanted a self-hosted solution with fast search and reliable external API integration.',
    ...
    stack: ['TypeScript', 'React', 'Express', 'Prisma', 'PostgreSQL', 'Redis', 'Zod', 'TanStack', 'Docker'],
    accent: '#64748b',
    size: 'small',
  ...
  },
  {
    ...
    description: 'Open-source competition platform with real-time leaderboards and SSE live activity feeds',
    ...
    problem: 'Open-source competition platforms need real-time leaderboards, issue tracking, and live activity feeds. Building this with polling is wasteful — SSE is the right tool but requires careful client management.',
    solution: 'Built a Next.js + Go platform with SSE real-time updates using a mutex-protected client registry and 100-message buffered channels with drop-on-full backpressure. Implemented a goroutine supervisor with panic recovery and 5s backoff throttling for zero-downtime live feeds. Added Valkey sorted-set leaderboards with compact float-score encoding across 10 categories.',
    lessons: [
    ...
    stack: ['Go', 'Gin', 'PostgreSQL', 'Redis', 'JWT', 'Next.js', 'React', 'TypeScript', 'Tailwind', 'Docker'],
    accent: '#84cc16',
    size: 'small',
  ...
  },
  {
    ...
    description: 'Zero-dependency Go CLI for concurrent CSV data processing with producer-worker-consumer pattern',
    ...
    problem: 'Processing large CSV survey datasets sequentially is slow. The standard library has all the tools for concurrent processing, but most developers reach for external libraries instead.',
    solution: 'Built a concurrent pipeline with 3 worker goroutines for CSV row parsing coordinated by sync.WaitGroup, decoupling I/O-bound reading from CPU-bound map construction. Implemented bounded buffered channels for backpressure between pipeline stages and a broadcast distributor fanning out processed rows to independent metric channels.',
    lessons: [
  ...
  },
  {
    ...
    longDescription: 'Built with TanStack Router, Vite, Tailwind CSS v4, and Motion. Every animation, every transition, every pixel was intentional. Features a command palette with fuzzy search, custom terminal cursor, kinetic text, responsive nav, and a library catalog.',
    problem: 'Most student portfolios look the same. Wanted something that feels like a technical journal, not a template.',
    solution: 'Designed from first principles — monospace meets serif, precise grid, intentional whitespace. No frameworks beyond the build tooling. Dark-only theme with custom cursor, fuzzy search, and per-page search behavior.',
    lessons: [
...
export const abandonedProjects = [
  {
    title: 'Chronos',
    description: 'A self-hostable timetable app built as a team project. I handled the React Native Expo frontend. The project stalled as the team lost interest and scope creep set in. My first steps into React Native — learned the ecosystem and what happens when a project outgrows its original vision.',
